...
const buy = (id) => {
  ...
  for (const item of cart) {
    if (foundProduct.id === item.id) {
      ...
      foundInCart = true;
      console.log(
        `${foundProduct.name} already in cart: incrementing quantity`
      );
      break;
    }
  }
...
const calculateTotal = () => {
  ...
  cart.forEach((item) => {
    ...
    if (
      (item.id === 1 && item.quantity >= 3) ||
      (item.id === 3 && item.quantity >= 10)
    ) {
  ...
  for (const item of cart) {
    if (item.id === 1 && item.quantity >= 3) {
      ...
      const subTotalWithDiscount =
        subTotalWithoutDiscount - subTotalWithoutDiscount * 0.2;
      item.subtotalWithDiscount = subTotalWithDiscount;
      console.log(
        `Promotion applied: 20% discount on ${item.name} if three or more units added to cart`
      );
    }
    if (item.id === 3 && item.quantity >= 10) {
      const subTotalWithoutDiscount = item.price * item.quantity;
      const subTotalWithDiscount =
        subTotalWithoutDiscount - subTotalWithoutDiscount * 0.3;
      item.subtotalWithDiscount = subTotalWithDiscount;
      console.log(
        `Promotion applied: 30% discount on ${item.name} if three or more units added to cart`
      );
    }
  }
};
